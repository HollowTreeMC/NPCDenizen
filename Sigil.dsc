...
SigilDBLoad:
    ...
    events:
        ...
        on player join flagged:!sigils:
        # create player db entry if they do not have the sigil flag
        ...
        on player join:
        # update the local flag for player on join
        - ~sql id:sigildb "query:SELECT sigils FROM sigils WHERE uuid='<player.uuid>'" save:join_query_result
        - flag <player> Sigils:<entry[join_query_result].result_list.get[1].get[1]>

        on delta time minutely:
        # update the local flags for all online players
        # for passing changes made by sigil commands run on other servers
...
#            # create an entry in the table if the player does not currently exist
#            - ~sql id:sigildb "query:SELECT sigils FROM sigils WHERE uuid='<[player_to_query].uuid>'" save:query_log_result
#
#            - if <entry[query_log_result].result_list.is_empty>:
#                - ~sql id:sigildb "update:INSERT INTO sigils VALUES ('<[player_to_query].uuid>',0);"
#                - debug '[Sigils] Created new entry for <[player_to_query].name>!'
#                # set player flag
#                - flag <[player_to_query]> Sigils:0
#            - else:
#                # set player flag
#                - flag <[player_to_query]> Sigils:<entry[query_log_result].result_list.get[1].get[1]>
#                - debug '[Sigils] Validated an entry for <[player_to_query].name>, set flag to <entry[query_log_result].result_list.get[1].get[1]>'
...
SigilDBQuery:
    ...
    script:
        - ~sql id:sigildb "query:SELECT sigils FROM sigils WHERE uuid='<[Player].uuid>'" save:query_result
        - define query_amount <entry[query_result].result_list.get[1].get[1]>
        - flag <[player]> Sigils:<[query_amount]>
        - determine <[query_amount]>
...
SigilCMD:
    ...
    - choose <context.args.first>:
        ...
        - case send:
            ...
            - if <[reciever].name> != <context.args.get[2]>:
                - narrate "<server.flag[sigiltag]> Error! Recipient <&e><context.args.get[2]> <&7>not found!"
                - stop
        ...
        - case set take give:
            ...
            # check input to see if player is valid, define playertag with <[query]> if input is validated
            - if !<context.args.get[2].exists>:
                - narrate "<server.flag[sigiltag]> Error! Recipient <&e><context.args.get[2]> <&7>not found!"
                - stop
            - else:
